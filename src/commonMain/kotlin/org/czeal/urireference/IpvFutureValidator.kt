 ...
 * 
 * 
 * Validates the `host` component of a URI reference as an IPvFuture address
 ...
 * 
 * <blockquote>
 * <pre style="font-family: 'Menlo', 'Courier', monospace;">`IPvFuture = "v" 1*HEXDIG "." 1*( unreserved / sub-delims / ":" ) `</pre>
</blockquote> * 
 * 
 * @see [RFC 3986,
 ...
 * @author Hideki Ikeda
 */
internal class IpvFutureValidator {
    /**
     ...
     */
    fun validate(ipvFuture: String) {
        // Ensure the host value is not empty.
        if (ipvFuture.isEmpty()) {
            throw org.czeal.urireference.Utils.newIAE(
        ...
        val segments: Array<String?> = ipvFuture.split("\\.".toRegex(), limit = 2).toTypedArray()
    ...
    private fun validateFirstSegmentOfIpvFuture(enclosed: String?, firstSegment: String) {
        // Check if the first segment starts with "v" or "V".
        val isValid = firstSegment.startsWith("v") || firstSegment.startsWith("V")
        ...
        // first segment .
        val version = firstSegment.substring(1)
        ...
        for (i in 0..<version.length) {
            ...
            if (!org.czeal.urireference.Utils.isHexDigit(c)) {
                throw org.czeal.urireference.Utils.newIAE(
                    "The host value \"[%s]\" is invalid because the content enclosed " +
                            "by brackets does not form a valid IPvFuture address due to " +
    ...
    private fun validateSecondSegmentOfIpvFuture(enclosed: String?, secondSegment: String) {
        ...
        for (i in 0..<secondSegment.length) {
            val c = secondSegment.get(i)

            if (org.czeal.urireference.Utils.isUnreserved(c) || org.czeal.urireference.Utils.isSubdelim(c) || c == ':') {
                continue
            }
