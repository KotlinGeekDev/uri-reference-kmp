...
class PortTest {
    @Test
    fun test_parse() {
        assertDoesNotThrow<Unit>( { PortValidator().validate(PORT1) })
        assertDoesNotThrow<Unit>( { PortValidator().validate(PORT2) })
        assertDoesNotThrow<Unit>( { PortValidator().validate(PORT3) })
